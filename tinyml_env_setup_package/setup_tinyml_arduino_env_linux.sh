...
# Stable TinyML Python environment for Linux
# Tested target: Ubuntu / Debian-like distributions
# Usage:
#   bash setup_tinyml_arduino_env_linux.sh
#   bash setup_tinyml_arduino_env_linux.sh tinyml-arduino ~/ai/projects
#
...
ensure_python311() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    msg "python3.11 not found. Attempting to install Python 3.11 with apt-get..."
    sudo apt-get update
  ...
  fail "Could not locate python3.11. Install Python 3.11 and python3.11-venv first, then rerun this script."
}

msg "Preparing stable TinyML environment on Linux: $ENV_NAME"
mkdir -p "$BASE_DIR"
