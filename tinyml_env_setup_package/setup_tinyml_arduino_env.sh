...
# Reusable setup script for a stable TinyML Python environment on macOS
# Focus: TensorFlow + TF-MOT + Jupyter kernel + smoke tests for
# quantization, pruning, QAT, distillation, and C-header export.
#
# Usage examples:
#   bash setup_tinyml_arduino_env.sh
#   bash setup_tinyml_arduino_env.sh tinyml-arduino ~/ai/projects
#   RUN_SMOKE_TEST=0 bash setup_tinyml_arduino_env.sh
#
# Optional environment variables:
#   RUN_SMOKE_TEST=0     Skip the smoke test at the end
#   REINSTALL=1          Delete and recreate the environment if it exists
#   INSTALL_ARDUINO_TFLM=1   Clone TFLite Micro Arduino examples library
...
# Pinned versions chosen for stability with TF-MOT on macOS
TF_VERSION="2.14.1"
...
ensure_python311() {
  ...
  if command -v brew >/dev/null 2>&1; then
    msg "python3.11 not found. Installing python@3.11 with Homebrew..."
  ...
  fail "Could not locate python3.11. Install it first, then rerun this script."
}

msg "Preparing stable TinyML environment: $ENV_NAME"
mkdir -p "$BASE_DIR"
